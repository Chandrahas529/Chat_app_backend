...
exports.getChat = async (req, res) => {
  ...
  const lastMessageId = req.query.initialMessageId;
  ...
  try {
    ...
    const result = await Message.find(query).sort({ _id: -1 }).limit(messageLimit); // imp
    const mappedResult = result.map((msg) => ({
      _id: msg._id.toString(),
      senderId:
        msg.senderId.toString() === userId ? null : msg.senderId.toString(),
      receiverId:
        msg.receiverId.toString() === userId ? null : msg.receiverId.toString(),
      ...
      forwardContent: msg?.forwardContent,
  ...
  try {
    ...
    for (const file of req.files) {
      ...
      const upload = await cloudinary.uploader.upload(filePath, {
        folder: isImage ? "vaarta_app/sendImages" : "vaarta_app/sendVideos",
